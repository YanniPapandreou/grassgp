...
# train_xs = Ws_train[:,0,0].copy()
# train_ys = Ws_train[:,1,0].copy()
# inds = np.where(train_xs < 0)[0]
# train_xs = train_xs.at[inds].set(train_xs[inds]*(-1))
# ax.scatter(train_xs, train_ys, color="blue", alpha=0.25, label='training points')
...
ax.fill_between(s_test, np.array(out_sample_pred_errors), np.array(upper_pred), color='lightblue', alpha=0.75, label=f'error + 1 std using preds')
ax.fill_between(s_test, np.array(out_sample_mean_errors), np.array(upper_mean), color='coral', alpha=0.75, label=f'error + 1 std using means')
ax.set_xlabel(r"$s$")
ax.legend()
ax.grid()
