...
# ╔═╡ ff8e9609-9db6-492f-a06e-fd43cfaf92cb
using Gridap, Plots, Distributions, Statistics, Random, LinearAlgebra, PlutoUI, NPZ

# ╔═╡ d762bb9e-4c99-4b6f-9db8-cf95438e2908
import Gridap: ∇

# ╔═╡ 94bd5621-c26f-4328-a240-d13ea5bf566a
md"""
...
$\log\kappa(x;p) := \sum_{k=1}^{d}\sqrt{2} p_{k} \cos(2\pi k x)$
"""

# ╔═╡ 42a535a5-15dd-434f-bac2-9f637b483bb2
function κ(x, p)
    d = length(p)
    C = cos.(2π*(0:(d-1))*x)
    log_k = √(2) * (p ⋅ C)
...
# ╔═╡ dd424651-c3fe-40d7-ad1c-8b502115faba
function dκdp(x,p,j)
    return √(2)*κ(x,p)*cos(2π*j*x)
end

# ╔═╡ 374f874d-a577-440a-8230-bc6949954dc9
...
# ╔═╡ d64f0bbe-cfbd-440a-9ad0-dfae3add539b
f(x) = 1.0

# ╔═╡ 6e26e190-0114-404a-9b5d-053146a153f3
p0 = [1.0, 2.0, 3.0]

# ╔═╡ c4bd50eb-2aeb-414e-a644-6cc69ae66ed6
@bind x0 Slider(0:0.01:1, default=0.5)

# ╔═╡ 31bf4280-a20e-4802-aecd-06bc4a1b7c1f
grads = dudp(p0, κ, dκdp, x0, f)

# ╔═╡ e7df30dd-4e0c-4b02-982d-89f025b1cdc9
# check that this agrees with the case kappa(x,p) = p constant
dudp([0.0], κ, dκdp, x0, f)/√2

# ╔═╡ 2a53dbe5-7aa1-48d0-ab2e-96e49cd55b83
...
# ╔═╡ 62f4a08d-9d5a-4945-baa9-83e2d7f7d0db
begin
	Random.seed!(42)
	d = 10
	M = 1000
...
# ╔═╡ 7bdeeea9-79da-406e-be1a-d9afd16c3f19
xs = 0.01:0.1:0.99

# ╔═╡ 843cad89-0d01-4d1e-b511-3f406032827a
AS_results = map(x -> getAS(x, ps, κ, dκdp, f), xs)

# ╔═╡ 3da6796c-6ba5-4fa2-b0b2-44c358693bae
plot(xs,map(tup -> tup[3], AS_results),ylim=(0.75,1.1))

# ╔═╡ 9f4fa60d-9ccf-4da4-b9bc-20086837b3c1
plot(xs,map(tup -> max(tup[2]...), AS_results))

# ╔═╡ 315144c8-164d-43c1-9dfa-05f3ba2663e4
Ws = map(tup -> tup[1], AS_results)

# ╔═╡ 49f6112d-987c-4834-9dfe-33d2251cae8d
Ws_mat = reduce(hcat, Ws)

# ╔═╡ 85e591cb-75ef-4cc5-bfd2-793296e8961b
for i in 1:10
	@assert Ws_mat[:,i] == Ws[i]
end

# ╔═╡ 74e67c5c-07e6-48bd-abd0-1eeb89672f76
...
# ╔═╡ 9f1d0e16-47c4-4bac-9414-0d9684d8ce33
npzwrite("xs.npz", xs)

# ╔═╡ 05fa0ea9-c42a-4477-85c9-551396d63c7c
npzwrite("Ws.npz", Ws_mat)

# ╔═╡ e0562638-2568-460d-9a63-000a9be4d86e
round.(Ws_mat, digits=2)
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "348bc11910766bfaf7bd4e99534b64d65a90ac1a"
