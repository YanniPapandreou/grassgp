...
# def grassmann_process(s, anchor_point, jitter=1e-06, proj_jitter=1e-4, L_jitter=1e-8, require_noise = False, reorthonormalize = True, b = 1.0):
...
#             K = rbf_covariance(s.reshape(n_s,-1), s.reshape(n_s,-1), var, length, noise, jitter=jitter)
#         else:
#             K = rbf_covariance(s.reshape(n_s,-1), s.reshape(n_s,-1), var, length, 0.0, jitter=jitter)
#
