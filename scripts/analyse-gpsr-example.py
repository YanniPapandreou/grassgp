...
# %%
if config.subsample_conf.random:
    sub_sample_key = random.PRNGKey(config.subsample_conf.seed)
    inds = random.choice(sub_sample_key, X_fine.shape[0], shape=(config.subsample_conf.n_x_samples,), replace=False)
else:
